#include <iostream>
#include <fstream>//made in visual studio  2022
#define SOCIALNETWORKCLASSES_H
using namespace std;
...
protected:
    Post** Timeline;
    HistoryEvents** HistoryOfTheUser;


public:
    void SeeYourHistory();
    void AddHistory(HistoryEvents* H);
    Object();
    char*& GetID();
    void SetID(char*);
    virtual void Display() = 0;
...
class Comment : public HistoryEvents
{
    char* commentID;
    char* commentText;
    Object* commentPostedBy;
    Post* commentOnThePost;
    static int noOfCommentsInSystem;
public:
    void  Display();
    void CommentOnThePost(Post*);
    Comment();
...
public:
    ...
    char*& GetDescription();
    virtual ~Post();
    void Reaction(Reacts*);

};
void Comment::Display()
{
    commentOnThePost->displayPost();
    cout << "Commented on the (" << commentText << ")" << endl;
}
int Post::GetTotalPost()
{
    return noOfPostsInSystem;
}

char* Post::GetIdOfThePost()
{
    return postID;
}
class Reacts;
...
public:
    ...
    void LoadAllReactionsOnThePost(const char*);
    void LoadAllData();
    void Run();
    ~Controller();
};

class Reacts :public HistoryEvents
{
private:
    Object* ReactedBy;
    Post* ReactedOn;
    int* reacttypeidentifier;
public:
    void GetInReactedBy(Object*);
    void Display();
    void display();
    void ReadDataFromTheFile(ifstream& fin);
    void GetReactedOn(Post* P);
};
void  Reacts::Display()
{

    ReactedOn->displayPost();
    cout << "Has Recated   ";
    if (*(reacttypeidentifier) == 0)
        cout << "(sad)";
    else if (*reacttypeidentifier == 1)
        cout << "(Happy)";
    else if (*reacttypeidentifier == 2)
        cout << "(neutral)";
    else if (*reacttypeidentifier == 3)
        cout << "(crying)";
    else if (*reacttypeidentifier == 4)
        cout << "(angry)";
    else
        cout << "--->Confused<---";
    cout << endl;
...
void Reacts::GetReactedOn(Post* P)
{
    ReactedOn = P;
}
void Reacts::GetInReactedBy(Object* U)
{
    ReactedBy = U;
}
void  Reacts::ReadDataFromTheFile(ifstream& fin)
{
    int TempVar;
    fin >> TempVar;
    reacttypeidentifier = new int(TempVar);



}
/// <class history to print history details of the user>
/// <only 1 print fn and 3 private class data members>
...
Object::Object() : TotalEvents(0), ID(nullptr), Timeline(nullptr), HistoryOfTheUser(nullptr) {};
...
void Object::AddHistory(HistoryEvents* H)
{
    if (HistoryOfTheUser == nullptr)
    {
        HistoryOfTheUser = new HistoryEvents * [20];
    }
    else
    {


        HistoryOfTheUser[TotalEvents] = H;
        TotalEvents++;
    }
...
void Comment::CommentOnThePost(Post* P)
{
    commentOnThePost = P;
...
Post::Post() :Reactionss(nullptr), postID(nullptr), description(nullptr), sharedBy(nullptr), likedBy(nullptr), comments(nullptr), activity(nullptr)
{
    noOfPostsInSystem++;
}
...
void Post::displayPost()
{
    ...
    if (TotalReactss != 0)
    {
        ...
        cout << "Recations on The Post: " << endl;
        for (int i = 0; i < TotalReactss; i++)
        {
            Reactionss[i]->display();
        }
    }
    else
        cout << "No User's Reacted On The Post" << endl;
...
void Reacts::display()
{
    ...
    if (*(reacttypeidentifier) == 0)
        ...
        cout << "--->invalid reaction<---";
    cout << endl;
}
void Post::Reaction(Reacts* R)
{

    TotalReactss++;
    Reacts** TempArray = new Reacts * [TotalReactss];
    for (int i = 0; i < TotalReactss; i++)
        TempArray[i] = new Reacts[TotalReactss];
    for (int i = 0; i < TotalReactss - 1; i++)
    {
        TempArray[i] = Reactionss[i];

    }

    TempArray[TotalReactss - 1] = R;
    delete[]Reactionss;
    Reactionss = TempArray;
...
void Controller::LoadUsersFromFile(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        cout << "Error Opening File Of Users\n";
}
void Controller::LoadPagesFromFile(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        cout << "Error Opening File Of Pages\n";
}
...
void Controller::LinkUsersAndPages(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        cout << "Error Opening File Of Link Pages And Users\n";
}
...
void Controller::LoadAllPostsFromFile(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        cout << "Error Opening File Of Posts\n";
}
...
void Controller::LoadAllCommentsFromFile(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        for (int i = 0; i < noOfComments; i++)
        {
            ...
            comment->CommentOnThePost(post);
            commentedBy->AddHistory(comment);
        }
    }
    else
        cout << "Error Opening File Of Commments\n";
}
...
void Controller::LoadAllActivitiesFromFile(const char* fileName)
{
    ...
    if (fin.is_open())
    {
        ...
        cout << "Error Opening File Of Activities\n";
}
...
void Controller::LoadAllData()
{
    ...
    LoadAllReactionsOnThePost("SocialNetworkReacts.TXT");
}
void Controller::LoadAllReactionsOnThePost(const char* filename)
{
    ifstream fin(filename);
    if (fin.is_open())
    {
        int TotalUsersInSys = 0;
        ...
        allReactions = new Reacts * [TotalNoOFreacts + 1];
        for (int j = 0; j < TotalNoOFreacts; j++)
        {
            int howmanyreacts;
            fin >> howmanyreacts;

            allReactions[j] = new Reacts[howmanyreacts];
            char Buffer[100];
            fin >> Buffer;

            int IndexOfThePost = -1;
            bool PostExists = false;
            int totalposts = Post::GetTotalPost();

            for (int i = 0; i < totalposts; i++)
            {
                if (strcmp(Buffer, allPosts[i]->GetIdOfThePost()) == 0)
                {
                    PostExists = true;
                    IndexOfThePost = i;
                    break;
            ...
            char UserReacted[100];
            if (PostExists == true)
            {
                for (int k = 0; k < howmanyreacts; k++)
                {
                    fin >> UserReacted;
                    ...
                    R->ReadDataFromTheFile(fin);

                    for (int i = 0; i < TotalUsersInSys; i++)
                    {
                        if (strcmp(UserReacted, allUsers[i]->GetID()) == 0)

                        {
                            R->GetInReactedBy(allUsers[i]);
                            allUsers[i]->AddHistory(R);
                        }

                    }
                    allReactions[k] = R;

                    allPosts[IndexOfThePost]->Reaction(R);
                    R->GetReactedOn(allPosts[IndexOfThePost]);
            ...
            {
                cout << "THIS POST DOESNT EXISTS" << endl;
            }

        }
        allReactions[TotalNoOFreacts] = nullptr;


    }
    else
        cout << "File Of The Reactios cannt be open" << endl << endl;
}
void Controller::Run()
{
    char dottedLine[] = "-------------------------------------------------------------------------------------------------------------\n";
    ...
    cout << endl << dottedLine << "Command :\tSet current user \"u7\"\n" << dottedLine;
    currentUser = SearchByID("u7", allUsers);
    if (currentUser)
    {
    ...
    postPtr = SearchByID("post7", allPosts);
    postPtr->AddComment(newComment1);

    cout << "Command:\tiew Post (post7)\n" << dottedLine;
    postPtr = SearchByID("post7", allPosts);
    if (postPtr)
        postPtr->displayPost();
